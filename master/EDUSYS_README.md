...
### 前端
- AdminLTE 4 (UI框架) - 本地文件 `/static/adminlte/`
- Bootstrap 5 - 使用国内BootCDN镜像
- Bootstrap Icons 1.13.1 - 使用国内BootCDN镜像
- Vanilla JavaScript
- 自定义API客户端

**CDN资源列表**：
- Bootstrap CSS: `https://cdn.bootcdn.net/ajax/libs/bootstrap/5.3.2/css/bootstrap.min.css`
- Bootstrap JS: `https://cdn.bootcdn.net/ajax/libs/bootstrap/5.3.2/js/bootstrap.bundle.min.js`
- Bootstrap Icons: `https://cdn.bootcdn.net/ajax/libs/bootstrap-icons/1.13.1/font/bootstrap-icons.min.css`
...
### 4. CDN资源加载失败（页面样式异常或加载慢）
**问题**：Bootstrap CSS/JS无法加载，导致页面样式错乱或模态框无法弹出

**解决方案**：
- 系统已配置使用国内BootCDN镜像源（https://cdn.bootcdn.net）
- 如果仍然无法访问，可能是网络问题，可以：
  1. 检查网络连接
  2. 尝试使用VPN或代理
  3. 将CDN资源下载到本地：
     ```bash
     # 下载Bootstrap CSS
     wget -P /path/to/static/bootstrap/css/ https://cdn.bootcdn.net/ajax/libs/bootstrap/5.3.2/css/bootstrap.min.css

     # 下载Bootstrap JS
     wget -P /path/to/static/bootstrap/js/ https://cdn.bootcdn.net/ajax/libs/bootstrap/5.3.2/js/bootstrap.bundle.min.js

     # 下载Bootstrap Icons
     wget -P /path/to/static/bootstrap/icons/ https://cdn.bootcdn.net/ajax/libs/bootstrap-icons/1.13.1/font/bootstrap-icons.min.css
     ```
  4. 修改index.html中的CDN链接为本地路径
